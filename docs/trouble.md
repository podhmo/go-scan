...
*   **複数パッケージのスキャン:**
    *   `-pkg` オプションで複数のパッケージをカンマ区切りなどで指定できるようにし、それらのパッケージを一度にスキャンして、マージされた `PackageInfo` を返す機能。

*   **依存関係の自動解決:**
    *   `@derivingconvert` などのアノテーションで指定された外部パッケージを、`go-scan` が自動的にスキャンし、型情報を解決してくれる機能。これにより、`main.go` で明示的に依存パッケージをスキャンする必要がなくなります。

*   **型変換のカスタマイズ:**
    *   `// convert:rule` のようなグローバルな変換ルールだけでなく、特定のフィールドに対して、より柔軟な変換ロジックを適用できる仕組み。例えば、ラムダ式や無名関数を直接アノテーションに記述できると便利です。

*   **エラーレポートの改善:**
    *   型が見つからない場合や、変換ルールが適用できない場合に、より詳細で分かりやすいエラーメッセージを出力する機能。どのファイルのどの行で問題が発生したのか、具体的な情報を提供してくれるとデバッグが容易になります。

*   **テスト用のユーティリティ:**
    *   インテグレーションテストを容易にするためのテスト用API。例えば、`go.mod` の情報をメモリ上から読み込んだり、一時的なモジュールルートを簡単に設定できるような機能があると、テストコードが書きやすくなります。

*   **プラグイン機構:**
    *   ユーザーが独自の変換ロジックやアノテーションを簡単に追加できるようなプラグイン機構。これにより、`go-scan` のコア機能を変更することなく、プロジェクト固有の要件に対応できます。
