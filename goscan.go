...
// WithConfig sets the shared configuration for the scanner.
// When this option is used, the scanner will use the components from the
// provided Config object instead of creating its own.
func WithConfig(config *Config) ScannerOption {
...
func New(options ...ScannerOption) (*Scanner, error) {
	...
	if s.config != nil {
		s.fset = s.config.Fset
		s.locator = s.config.Locator
		s.Logger = s.config.Logger
		s.overlay = s.config.Overlay
	}
