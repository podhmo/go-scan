...
func (e *Evaluator) evalSelectorExpr(n *ast.SelectorExpr, env *object.Environment, pkg *scanner.PackageInfo) object.Object {
	...
	case *object.Package:
		// Handle package-level function calls, checking for intrinsics before lazy loading.
		key := val.Path + "." + n.Sel.Name
		if intrinsicFn, ok := e.intrinsics.Get(key); ok {
			e.logger.Debug("evalSelectorExpr: found package intrinsic", "key", key)
		...
		if val.Env.IsEmpty() {
			...
			for _, f := range pkgInfo.Functions {
				if ast.IsExported(f.Name) {
					...
					if intrinsicFn, ok := e.intrinsics.Get(key); ok {
						val.Env.Set(f.Name, &object.Intrinsic{Fn: intrinsicFn})
					} else {
			...
			for _, v := range pkgInfo.Variables {
				if v.IsExported {
					val.Env.Set(v.Name, &object.SymbolicPlaceholder{Reason: fmt.Sprintf("external var %s.%s", val.Name, v.Name)})
				}
			}
			...
			for _, t := range pkgInfo.Types {
				if ast.IsExported(t.Name) {
					val.Env.Set(t.Name, &object.SymbolicPlaceholder{Reason: fmt.Sprintf("external type %s.%s", val.Name, t.Name)})
				}
			}
		}
