...
- [x] **Const resolution from test packages**: Worked around an issue where package-level `const` values were not resolved by `symgo` when the analysis was initiated from a test package (`--include-tests`). The workaround is to change the `const` to a `var`, which is correctly resolved.
