...
func TestRecursion_selector(t *testing.T) {
	source := `
package main
...
`
	...
	if !strings.Contains(err.Error(), "call stack depth exceeded") {
		t.Fatalf("expected error to contain 'call stack depth exceeded', but got %q", err.Error())
	}
}
...
`,
			ShouldFail:    true,
			ExpectedError: "call stack depth exceeded",
		},
		{
