...
- **Find Orphan Functions and Methods**: A new tool `examples/find-orphans` that uses the improved `symgo` engine to perform whole-program analysis and identify unused functions and methods. It supports multi-module workspaces and `//go:scan:ignore` annotations. It intelligently detects whether to run in "application mode" (starting from `main.main`) or "library mode" (starting from all exported functions).
    - **[x]** **Workspace-Scoped Scanning**: Fixed a critical bug where the symbolic execution engine would attempt to scan packages outside the defined workspace (e.g., standard library packages) during type resolution, causing unexpected panics. The `symgo` interpreter now accepts a `WithScopeCheck` function, allowing `find-orphans` to enforce a strict workspace boundary and prevent out-of-scope scanning. ([docs/trouble-find-orphans-scan-external.md](./docs/trouble-find-orphans-scan-external.md))
