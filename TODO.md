...
- [x] **Fix non-slice response schema generation in `docgen`**: While fixing the slice response issue, a regression was introduced where handlers returning single struct instances (e.g., `func() User`) no longer have their response schemas generated correctly. This was traced to the symbolic evaluator halting prematurely on certain Go statements (multi-value assignment, field assignment). The evaluator has been enhanced to handle these cases, and the `docgen` tests now pass. See [docs/trouble-docgen.md](./docs/trouble-docgen.md) for original details.
