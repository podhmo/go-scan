...
- **Type-Safe `docgen` Patterns**: Enhanced the `docgen` tool to allow defining analysis patterns using type-safe function and method references (`Fn: mypkg.MyFunc`, `Fn: (*mypkg.MyType)(nil).MyMethod`) instead of string keys. This involved creating new `GoSourceFunction` and `GoMethodValue` objects in the `minigo` interpreter that preserve the necessary definition context, ensuring correct symbol and method resolution. ([docs/plan-docgen-minigo-fn-ref.md](./docs/plan-docgen-minigo-fn-ref.md))
