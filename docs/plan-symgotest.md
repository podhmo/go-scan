...
*   **Pre-Run Interpreter Configuration**: The `symgotest.Run` function is designed for test cases where the setup is entirely handled by the `TestCase` and `Option` structs. It does not currently provide a hook to perform arbitrary configuration on the `symgo.Interpreter` instance *before* the entry point is executed. This makes it difficult to refactor tests that rely on imperative setup steps, such as `interpreter.BindInterface`, which must be called before analysis begins. Such tests cannot be converted to `symgotest.Run` without extending the library with new options.
