...
- [x] **`find-orphans`: Advanced Usage Analysis (Interfaces)**: The `symgo` engine and `find-orphans` tool have been enhanced to allow for more precise analysis of interface method calls. The `symgo` evaluator now correctly tracks the set of possible concrete types for an interface variable, even across control-flow branches (`if/else`). This was achieved by implementing a copy-on-write semantic for variable assignments within nested scopes and merging the resulting states. See [docs/trouble-find-orphans.md](./docs/trouble-find-orphans.md) for a detailed analysis and the final resolution.
