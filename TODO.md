...
- [ ] **Mismatched Package Names**: The interpreter cannot correctly resolve types from unaliased imports where the package name differs from the import path's base name (e.g., `import "gopkg.in/yaml.v2"` which is `package yaml`). This is due to a limitation in the underlying `go-scan` library's static type resolver (`TypeInfoFromExpr`), which does not have access to the dynamic package name information that `symgo` discovers. See [docs/trouble-symgo-mismatch-import-package-name.md](./docs/trouble-symgo-mismatch-import-package-name.md) for a detailed analysis.
