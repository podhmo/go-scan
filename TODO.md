...
- [x] **Step 3: User Extensibility**
    - [x] **Implement `minigo`-based Pattern Loader**: Create a loader in `docgen` that reads a standard Go script (`patterns.go`) and parses a list of pattern definitions using `minigo.EvalFile` and `minigo.Result.As`. This provides a better user experience with Go tooling support.
    - [x] **Integrate Pattern Loader with Analyzer**: Modify the `docgen.Analyzer` to dynamically register intrinsics based on the patterns loaded from the Go script at startup.
