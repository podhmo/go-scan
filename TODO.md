...
- **Find Orphan Functions and Methods**: A new tool `examples/find-orphans` that uses the improved `symgo` engine to perform whole-program analysis and identify unused functions and methods. It supports multi-module workspaces and `//go:scan:ignore` annotations. It intelligently detects whether to run in "application mode" (starting from `main.main`) or "library mode" (starting from all exported functions).
    ...
    - **[x]** **Update Library Mode Logic**: The logic for `--mode=lib` has been updated. Previously, it reported uncalled exported functions as orphans. The new specification requires that all exported functions in the analysis scope are treated as entry points and are therefore never reported as orphans. This was fixed by modifying the analysis loop to mark all library-mode entry points as "used" by default.
