...
func TestFindOrphans_usageScope(t *testing.T) {
	// This test ensures that when a function is passed as an argument, it is
	// only marked as "used" if it belongs to a package that is within the
	// analysis scope. A function from an external package passed as an argument
	// should be ignored.
	files := map[string]string{
		"go.mod": "module example.com/usage-scope-test\ngo 1.21\n",
		"main.go": `
package main
...
// A local function that should be marked as "used" when passed to wrapper.
func localFunc(s string) string {
	return s
}

// An unused local function that should be an orphan.
func orphanFunc(s string) string {
	return s
}

func main() {
	// Use the local function. It should not be an orphan.
	wrapper(localFunc)

	// Use a function from an external package. This should not affect the analysis.
...
`,
	...
	if strings.Contains(output, "localFunc") {
		t.Errorf("localFunc was reported as an orphan, but it was used as a method value argument")
	}

	expectedOrphan := "example.com/usage-scope-test.orphanFunc"
