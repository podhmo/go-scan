...
### 2.1. 既存ドキュメント (`sketch/ja/from-*.md`) からの洞察

リポジトリ内の `sketch/ja/from-derivingbind.md` および `docs/ja/from-derivingjson.md` は、個別のジェネレータ開発経験に基づき、`go-scan` への多くの貴重な改善提案を含んでいます。これらの提案は、複数ジェネレータが協調するシナリオにおいて、その重要性が一層高まります。
...
### 2.5. `ExternalTypeOverride`: 依存ライブラリ型解釈の制御とスキャン範囲の最適化

`go-scan` の `scanner.ExternalTypeOverride` 機能（`scanner.New` の第二引数で渡せる `map[string]string`）は、前述の「eager vs lazy」の議論や、「アプリケーションコード中心のスキャン」という思想と深く関連します。これは、ジェネレータが特定の外部パッケージの型をどのように解釈するかを明示的に指示する仕組みです。

**役割と意義:**

1.  **依存ライブラリ型に対する「スタブ」または「解釈の強制」**:
    *   例えば、`github.com/google/uuid.UUID` 型を常に `string` として扱いたい場合、`ExternalTypeOverride` に `{"github.com/google/uuid.UUID": "string"}` と登録します。
    *   `go-scan` は `uuid.UUID` に遭遇した際、この設定に基づき `FieldType.Name` を `"string"` とし、`FieldType.IsResolvedByConfig = true` のようなフラグを立てます。
    *   これにより、`go-scan` は `uuid.UUID` の実際のソースコードをスキャンする必要がなくなり、「手動スタブ登録」に近い効果が得られます。現在の実装は型名を指定文字列に置き換えるものですが、ジェネレータが特定の外部型を「ある基本型と同等に扱える」という情報だけで十分なケースは多くあります（例: `uuid.UUID` を `string` としてパースする）。

2.  **スキャン対象の削減とパス解決の簡略化への貢献**:
    *   一般的な外部型（例: `time.Time`）の解釈を事前定義することで、それらのライブラリのソースコードスキャンを回避できます。
    *   これは「アプリケーションコードのみを主にスキャンし、依存ライブラリはシグネチャレベル（または事前定義された解釈）で済ませる」というアプローチを支援し、`go list` 等の外部ツールへの依存度や代替実装の必要性を低減する効果が期待できます。READMEで触れられていた「locatorの制限」への対応策の一つとも言えます。

3.  **ジェネレータの柔軟性と制御性の向上**:
    *   ジェネレータ開発者は、特定の外部型を一貫した方法で扱うために `go-scan` の型解釈をカスタマイズできます。

`ExternalTypeOverride` は、`go-scan` が「アプリケーションコード中心のスキャン」と「依存ライブラリの効率的な取り扱い」のバランスを取るための重要な機能です。不要なライブラリスキャンの回避（eagerな末端探索の回避）、外部ツールへの依存低減、ジェネレータによる型解釈の一貫性確保といったメリットをもたらします。
