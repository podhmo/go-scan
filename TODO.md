...
- [x] **Bugfix: External Type Resolution**:
    ...
    - [x] Modify the `symgo` evaluator and/or `scanner` to ensure that unresolved types are consistently represented as symbolic type placeholders, not functions. (Note: The root cause was found to be in `goscan.Scanner.FindSymbolInPackage`, which was scanning package files one by one, leading to incomplete type information. This has been fixed to scan all of a package's files at once.)
    - [x] Add a regression test to `symgo` that attempts to use an external type and fails if the "invalid indirect" error occurs. (Note: A new test `TestFindOrphans_MultiFilePackageSymbolResolution` was added to `examples/find-orphans` to cover this multi-file resolution scenario.)
