...
- **Developer Experience & Testing**:
    ...
    - **`scantest`: Path to Import Path Conversion**: A helper (`scantest.PathConverter`) to automatically convert filesystem path patterns (like `.`) into their corresponding Go import path patterns, simplifying test setup for tools that consume import paths.
