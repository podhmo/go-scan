...
func (i *Interpreter) Eval(ctx context.Context, node ast.Node, pkg *scanner.PackageInfo) (Object, error) {
	// The evaluator itself handles import processing on a file-by-file basis,
	// so we no longer need to pre-populate the environment here. This was a source
	// of bugs as it only used a "representative file".
