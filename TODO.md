...
- [ ] **Fix non-slice response schema generation in `docgen`**: While fixing the slice response issue, a regression was introduced where handlers returning single struct instances (e.g., `func() User`) no longer have their response schemas generated correctly. See [docs/trouble-docgen-non-slice-response.md](./docs/trouble-docgen-non-slice-response.md) for details. (Note: The test for this is currently disabled in `main_test.go`.)
