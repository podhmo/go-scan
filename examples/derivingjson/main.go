...
func Generate(ctx context.Context, gscn *goscan.Scanner, pkgInfo *scanner.PackageInfo) error {
	...
	for _, typeInfo := range pkgInfo.Types {
		...
		for _, field := range typeInfo.Struct.Fields {
			...
			if field.Type.GetFullImportPath() == "" {
			...
			if isInterfaceField {
				...
				if interfaceDef != nil { // Resolved interface
					if field.Type.GetFullImportPath() != "" && field.Type.GetFullImportPath() != pkgInfo.ImportPath {
						interfaceDefiningPkgImportPath = field.Type.GetFullImportPath()
				...
				if resolvedFieldType != nil && resolvedFieldType.Name != "" { // resolvedFieldType.Name can be empty for unnamed types like `struct{}`
					...
					} else if field.Type.GetFullImportPath() != "" { // Fallback to FieldType's import path if FilePath on resolved is empty
						definingPkgPath = field.Type.GetFullImportPath()
					}
					...
					typeName = importManager.Qualify(field.Type.GetFullImportPath(), field.Type.Name)
				}
