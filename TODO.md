...
- **`symgo` Evaluator Enhancements**: The symbolic execution engine's evaluator has been significantly improved, resolving critical bugs that previously blocked call-graph analysis tools.
    ...
    - **Advanced Call-Graph Analysis**: The evaluator now correctly analyzes function calls that are nested within other calls (e.g., `f(g())`) and correctly identifies functions that are passed as arguments (e.g., `f(g)`). This was achieved by fixing bugs in block statement evaluation and improving how intrinsics are discovered and applied, leading to more accurate usage detection in tools like `find-orphans`.
