...
- [x] **Bugfix: External Type Resolution**:
    ...
    - [x] Modify the `symgo` evaluator and/or `scanner` to ensure that unresolved types are consistently represented as symbolic type placeholders, not functions. (Note: The root cause was that `symgo`'s evaluator was using a file-by-file scanner (`FindSymbolInPackage`) which was not suitable for its whole-package analysis needs. The evaluator has been changed to use `ScanPackageByImport` to ensure complete package information is available before symbol resolution.)
    - [x] Add a regression test to `symgo` that attempts to use an external type and fails if the "invalid indirect" error occurs. (Note: A new test `TestFindOrphans_MultiFilePackageSymbolResolution` was added to `examples/find-orphans` to cover the multi-file resolution scenario that was failing.)
