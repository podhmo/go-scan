...
- [ ] **Add support for string indexing**: Enhance the evaluator to handle the index operator (`s[i]`) on string objects. This is required for direct source interpretation of the `strings` package.
...
- [ ] **Implement Distinct Integer Types**: Introduce distinct object types for different integer kinds (e.g., `object.Uint8`, `object.Uint64`) to correctly handle unsigned integers and overflow, as described in the "Fundamental Design Limitations" section.
