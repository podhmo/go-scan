...
- **`symgo`: Correct Package Name Resolution**: Fixed a critical bug where the interpreter incorrectly assumed a package's name was the last part of its import path. It now correctly resolves the name from the `package <name>` declaration in the source files, enabling analysis of packages like `gopkg.in/yaml.v2` and `github.com/mattn/go-isatty`.
