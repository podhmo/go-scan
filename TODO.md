...
- **Find Orphan Functions and Methods**: A new tool `examples/find-orphans` that uses the improved `symgo` engine to perform whole-program analysis and identify unused functions and methods. It supports multi-module workspaces and `//go:scan:ignore` annotations. It intelligently detects whether to run in "application mode" (starting from `main.main`) or "library mode" (starting from all exported functions).
    - **[~]** **Specification and Logic Refinement**: The specification for the tool's behavior, particularly for library mode, has been significantly clarified and documented in `examples/find-orphans/spec.md`. The core logic has been updated to match. A bug related to intra-package method analysis has been fixed in `symgo`, but a separate upstream bug in `go-scan`'s file filtering prevents full correctness. See `docs/trouble-find-orphans-spec.md` for details.
