...
- **Verification**: The `Recursive` function in `tools/goinspect/testdata/src/myapp/main.go` should be correctly identified and labeled in the updated `default.golden` file.

## Initial Analysis

1.  **Code Location**: The core logic for printing the call graph is in `tools/goinspect/main.go`, specifically within the `Printer.printRecursive` method.
...
- **Enabled Memoization**: In `tools/goinspect/main.go`, the call to `symgo.NewInterpreter` was updated to include the `symgo.WithMemoization(true)` option. This should improve performance for larger analyses.
