...
### 2.5. minigo: 自体の機能的な制約

`docgen`の拡張性の提案で`minigo`の利用を推奨しましたが、その`minigo`エンジン自体にもGo言語の全ての機能を網羅しているわけではない、という機能的な制約が存在します。`minigo/evaluator/evaluator.go`を調査した結果、以下の点が明らかになりました。

*   **並行処理の完全な欠如**:
    - `go`キーワードによるgoroutineの起動はサポートされていません。
    - `chan`（チャネル）型は実装されておらず、`close`ビルトイン関数は常にエラーを返すスタブです。
    - `select`文も実装されていません。
*   **型システムのギャップ**:
    - **型アサーションと型スイッチの欠如**: `value.(T)`という型アサーションや、`switch v := i.(type)`という型スイッチ構文はサポートされていません。これはGoの型システムの中核的な機能であり、大きなギャップと言えます。
    - **インターフェイスのメソッドセットチェックが不完全**: インターフェイスを実装しているかどうかのチェックがメソッドの数のみで行われ、引数や戻り値の型を比較していません。
*   **標準ライブラリのバインディング不足**:
    FFIを通じてどのGoのパッケージでも利用可能ですが、`fmt.Sprintf`のような頻繁に使われる標準ライブラリ関数を利用するには、利用者が手動で`interpreter.Register()`を呼び出して登録する必要があります。

これらの制約は、`minigo`を「汎用的なGoスクリプト実行エンジン」として利用する際には注意が必要ですが、「設定ファイル」として利用する`docgen`のユースケースにおいては、現状の実装で十分な機能を持っていると言えます。
