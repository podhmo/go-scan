...
- **Core Scanning Engine**: A robust, AST-based engine for parsing Go code. It supports lazy, on-demand, cross-package type resolution, and correctly handles complex scenarios like recursive types and generic type definitions. It can extract detailed information about all major Go constructs, including structs, functions, interfaces, and constants.
    ...
    - **Robust Package Name Resolution**: The scanner can now correctly resolve the declared package name for any import path, even when the package name differs from the last element of the path (e.g., `gopkg.in/yaml.v2`). This is handled by a new, cached, and policy-aware `ResolvePackageName` method, which prevents incorrect `identifier not found` errors in `symgo`.
