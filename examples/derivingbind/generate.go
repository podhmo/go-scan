...
func Generate(ctx context.Context, gscn *goscan.Scanner, pkgInfo *scanner.PackageInfo) error {
	...
	for _, typeInfo := range pkgInfo.Types {
		...
		// This needs to be defined within the main package or passed in.
		// For now, we assume bindMethodTemplateString is available globally.
		// A better approach would be to pass it as an argument to Generate.
