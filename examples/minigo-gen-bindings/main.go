...
func Install(interp *minigo.Interpreter) {
	...
	{{- else }}
	// No symbols to register for {{ .PackagePath }}.
...
func generate(ctx context.Context, s *goscan.Scanner, outputDir, pkgPath string) error {
	...
	for _, f := range pkgInfo.Functions {
		if f.Receiver == nil && f.AstDecl != nil && f.AstDecl.Name != nil && f.AstDecl.Name.IsExported() {
			// Skip generic functions, as the interpreter doesn't support them.
			if f.AstDecl.Type.TypeParams != nil {
				slog.DebugContext(ctx, "skipping generic function", slog.String("package", pkgPath), slog.String("function", f.Name))
				continue
			}
