...
**Observation**: The `run_in_bash_session` tool does not always have a predictable Current Working Directory (CWD). It may unexpectedly be set to a subdirectory (e.g., `/app/examples/find-orphans`) instead of the project root (`/app`).

**Impact**: This can cause commands like `go test ./...` or `make test` to behave incorrectly, as their scope of operation is limited to the unexpected CWD. It can also cause file path lookups (e.g., `lstat ./symgo/...`) to fail.

**Best Practice**:
*   When a command fails with a "file not found" or unexpected scope error, the first diagnostic step should be to run `pwd`.
*   To ensure reliable execution of project-wide commands, use the canonical `make` targets (`make test`, `make format`) as defined in `AGENTS.md`. These are designed to be run from the root.
*   If manual commands are necessary, it is a good practice to start the `run_in_bash_session` with `cd /app && ...` to guarantee the CWD is the project root.

### 2. Testing Memoization Logic Without Interference

**Observation**: When testing a memoization feature that caches results based on `*object.Function` pointers, it is critical that the test does not prevent the `*object.Function` from being created in the first place.

**Impact**: An initial attempt to test the `symgo` memoization feature involved registering an `*object.Intrinsic` to override the target function (`NewService`) and count its calls. However, this was a logical flaw:
*   The intrinsic system in `symgo` is a complete override. When the evaluator encounters a call to `NewService`, it immediately finds and returns the `*object.Intrinsic`.
*   The `applyFunction` wrapper, where the memoization logic resides, receives the `*object.Intrinsic`, not the original `*object.Function`.
*   The memoization cache check, which was keyed on `*object.Function` pointers, was therefore never triggered. The test was invalidating its own premise.

**Best Practice**:
*   To test if a function's body is being correctly memoized, do not replace the function itself.
*   Instead, introduce a "hook" or "spy" function inside the body of the function being tested.
*   Register the test-specific intrinsic on the hook function. This allows the original function to be resolved to a `*object.Function`, flow through the memoization logic correctly, while still allowing the test to count how many times its body is actually executed.

**Example**:
```go
// Source code under test
func Tally() {} // The hook function
func MyFunctionToMemoize() {
    Tally() // Call the hook from inside
    // ... rest of the function logic
}

// Test code
var callCount int
interpreter.RegisterIntrinsic("...Tally", func(...) { callCount++ })
// Now, when MyFunctionToMemoize is called, the memoization logic for it will work,
// and the test can verify its behavior by checking callCount.
```
This pattern ensures that the test validates the target mechanism without inadvertently bypassing it.
