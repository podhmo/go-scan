...
// WithModuleWalkerConfig sets the shared configuration for the walker.
// When this option is used, the walker will use the components from the
// provided Config object instead of creating its own.
func WithModuleWalkerConfig(config *Config) ModuleWalkerOption {
...
func NewModuleWalker(options ...ModuleWalkerOption) (*ModuleWalker, error) {
	...
	// If a config is provided, use its components.
	if w.config != nil {
		w.fset = w.config.Fset
		w.locator = w.config.Locator
		w.Logger = w.config.Logger
		w.overlay = w.config.Overlay
	}

	// Initialize components if they were not provided via a config.
	if w.fset == nil {
	...
	internalScanner, err := scanner.New(w.fset, nil, w.overlay, w.locator.ModulePath(), w.locator.RootDir(), nil, false, w.Logger)
	if err != nil {
