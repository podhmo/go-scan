...
`,
	...
	log.SetOutput(w)
	err := run(context.Background(), true, false, dir, false, false, "auto", 0, startPatterns, []string{"testdata", "vendor"}, nil, nil)
	if err != nil {
...
`,
	...
	err = run(context.Background(), true, false, ".", false, false, "auto", 0, startPatterns, []string{"testdata", "vendor"}, nil, nil)
	if err != nil {
		t.Fatalf("run() failed: %v", err)
	}
...
`,
	...
	err := run(context.Background(), true, true, dir, false, false, "lib", 0, startPatterns, []string{"testdata", "vendor"}, nil, nil)
	if err != nil {
		t.Fatalf("run() failed: %v", err)
	}
...
`,
	...
	err = run(context.Background(), true, false, ".", false, false, "auto", 0, startPatterns, []string{"testdata"}, nil, nil)
	if err != nil {
		t.Fatalf("run() failed: %v", err)
	}
...
`,
	...
	err := run(context.Background(), true, false, dir, false, false, "lib", 0, startPatterns, []string{"testdata", "vendor"}, nil, nil)
	if err != nil {
		t.Fatalf("run() failed: %v", err)
	}
...
`,
	...
	err := run(context.Background(), true, false, dir, false, false, "auto", 0, startPatterns, []string{"vendor"}, nil, nil)
	if err != nil {
		t.Fatalf("run() failed: %v", err)
	}
...
`,
	...
	// Set verbose to false, and asJSON to false
	err = run(context.Background(), true, false, workspaceRoot, false, false, "auto", 0, startPatterns, []string{"testdata", "vendor"}, nil, nil)
	if err != nil {
		t.Fatalf("run() failed: %v", err)
	}
...
`,
	...
	t.Run("with include-tests=true", func(t *testing.T) {
		...
		err := run(context.Background(), true, true, dir, true, false, "auto", 0, []string{"./..."}, nil, nil, nil)
		if err != nil {
			t.Fatalf("run() failed: %v", err)
		}
	...
	t.Run("with include-tests=false", func(t *testing.T) {
		...
		err := run(context.Background(), true, false, dir, true, false, "auto", 0, []string{"./..."}, nil, nil, nil)
		if err != nil {
			t.Fatalf("run() failed: %v", err)
		}
...
`,
	...
	err := run(context.Background(), true, false, dir, false, true, "auto", 0, startPatterns, []string{"testdata", "vendor"}, nil, nil)
	if err != nil {
		t.Fatalf("run() failed: %v", err)
	}
...
`,
	...
	// Force library mode
	err = run(context.Background(), true, false, "", false, false, "lib", 0, startPatterns, nil, nil, nil)
	if err != nil {
		t.Fatalf("run() failed: %v", err)
	}
...
`,
	...
	err = run(context.Background(), true, false, "", false, false, "app", 0, startPatterns, nil, nil, nil)
	if err == nil {
...
func TestFindOrphans_WithPrimaryAnalysisScope(t *testing.T) {
	files := map[string]string{
...
`,
	...
	err := run(context.Background(), true, false, dir, false, false, "lib", 0, startPatterns, nil, nil, primaryScope)
	if err != nil {
		t.Fatalf("run() failed: %v", err)
	}
	...
	// ASSERT: Only the orphan from the primary scope (pkga) should be reported.
	if !strings.Contains(output, "UnusedInA") {
		t.Errorf("expected 'UnusedInA' to be reported as an orphan, but it was not.\nOutput:\n%s", output)
	}
	if strings.Contains(output, "UnusedInB") {
		t.Errorf("'UnusedInB' was reported as an orphan, but it should have been ignored.\nOutput:\n%s", output)
	}

	// Verify the exact orphan list
	expectedOrphans := []string{
		"example.com/test/pkga.UnusedInA",
	}
	sort.Strings(expectedOrphans)
