...
- [ ] **Mismatching Import Path and Package Name**: The interpreter's import resolution has been improved to handle packages like `gopkg.in/yaml.v2`, but a follow-on bug was discovered related to the resolution of built-in types (e.g., `string`), which prevents the fix from being fully validated. ([docs/trouble-symgo-mismatch-import-package-name.md](./docs/trouble-symgo-mismatch-import-package-name.md))
- [ ] **Resilience to Undefined Identifiers**: The interpreter has been updated to create placeholders for undefined identifiers in out-of-policy packages. This is partially working, but its full validation is blocked by the built-in type resolution bug. ([docs/trouble-symgo-mismatch-import-package-name.md](./docs/trouble-symgo-mismatch-import-package-name.md))
