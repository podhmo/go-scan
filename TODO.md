...
- **`symgo`: Robust Interface Resolution**: Implemented a two-phase analysis to correctly resolve method calls on interface types. The evaluator now records all interface method calls during an initial pass. A new `Finalize()` method, called after evaluation, then finds all concrete types that implement the called interfaces and marks the corresponding methods as "used". This fixes a class of bugs that caused false positives in tools like `find-orphans`. The fix involved a major refactoring of `extendFunctionEnv` to use pre-scanned `FunctionInfo` for reliable parameter type resolution. See ([docs/plan-symgo-interface-resolution.md](./docs/plan-symgo-interface-resolution.md)) for details.
