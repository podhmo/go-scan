...
- **`symgo`: Robust Package Name and Shallow Scan Handling**: The `symgo` engine's package handling has been significantly improved. It now correctly resolves a package's identifier from its source `package` clause, fixing errors for packages like `github.com/mattn/go-isatty` where the package name differs from its import path. Additionally, method calls on symbolic placeholders (used for packages outside the scan policy) are now handled gracefully, preventing analysis failures.
