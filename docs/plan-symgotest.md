...
*   **Explicit Policy for Multi-Module**: While `symgotest` handles `go.work` and `replace` directives for module discovery, the *default scan policy* still only includes the entry point's package. For tests that span multiple local modules (e.g., a `main` module calling a `helper` module), an explicit `symgotest.WithScanPolicy` option is required to ensure all necessary source code is analyzed. A common pattern is to use a simple prefix-based policy, e.g., `symgotest.WithScanPolicy(func(p string) bool { return strings.HasPrefix(p, "example.com/") })`.
