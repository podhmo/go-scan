...
func Install(interp *minigo.Interpreter) {
	interp.Register("{{ .PackagePath }}", map[string]any{
		...
		"{{ . }}": {{ $.PackageName }}.{{ . }},
		{{- end }}
		{{- range .StructTypeSymbols }}
		"{{ . }}": {{ $.PackageName }}.{{ . }}{},
		{{- end }}
		{{- range .InterfaceTypeSymbols }}
		"{{ . }}": (*{{ $.PackageName }}.{{ . }})(nil),
		{{- end }}
	})
}
...
func generate(ctx context.Context, s *goscan.Scanner, outputDir, pkgPath string) error {
	...
	structTypeSymbols := make(map[string]struct{})
	interfaceTypeSymbols := make(map[string]struct{})
	...
	for _, t := range pkgInfo.Types {
		if t.Name != "" && ast.IsExported(t.Name) {
			if t.Kind == scanner.StructKind {
				structTypeSymbols[t.Name] = struct{}{}
			} else {
				interfaceTypeSymbols[t.Name] = struct{}{}
			}
		}
	}
	...
	structTypeSymbolSlice := make([]string, 0, len(structTypeSymbols))
	for s := range structTypeSymbols {
		structTypeSymbolSlice = append(structTypeSymbolSlice, s)
	}
	sort.Strings(structTypeSymbolSlice)

	interfaceTypeSymbolSlice := make([]string, 0, len(interfaceTypeSymbols))
	for s := range interfaceTypeSymbols {
		interfaceTypeSymbolSlice = append(interfaceTypeSymbolSlice, s)
	}
	sort.Strings(interfaceTypeSymbolSlice)
	...
	params := struct {
		...
		PackagePath          string
		ValueSymbols         []string
		StructTypeSymbols    []string
		InterfaceTypeSymbols []string
	}{
		PackageName:          pkgName,
		PackagePath:          pkgPath,
		ValueSymbols:         valueSymbolSlice,
		StructTypeSymbols:    structTypeSymbolSlice,
		InterfaceTypeSymbols: interfaceTypeSymbolSlice,
	}
